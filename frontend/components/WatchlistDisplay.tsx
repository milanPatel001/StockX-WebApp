...
import { WatchList, WatchlistStock } from "@/backend";

function WatchlistDisplay({ watchlist }: WatchList) {
  const router = useRouter();

  const [st, setStocks] = useState<WatchlistStock[]>([]);
  ...
  const handleDelete = async (sym: string) => {
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_DEV_API_URL}/api/watchlist/${userLoggedIn?.uid}/remove/${sym}`,
        options
      );
    ...
    <div>
      <div className="border-2 mx-auto border-stone-300 rounded-lg pt-2 px-4 pb-5 bg-white">
        ...
        <table className="w-full h-full rounded-xl text-left">
          <tbody>
            {st
              ?.sort((a: WatchlistStock, b: WatchlistStock): number => {
                type props =
                  | "stockName"
                  | "stockPercentChange"
                  | "stockPrice"
